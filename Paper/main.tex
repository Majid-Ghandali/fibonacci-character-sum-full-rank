...
interpretation. By \Cref{lem:rank-order} below, it is equivalent 
to $-r^2$ being a primitive root of $\F_p^\times$, where $r$ is 
either root of $x^2-x-1$ in $\F_p$. The condition is thus a 
\emph{Fibonacci analogue} of the kind of primitivity condition 
studied in Artin's primitive root conjecture. The non-obvious 
content of \Cref{lem:G} below is that, given the parity 
consequence $p\equiv 3\pmod 4$ which itself follows from 
$\alpha(p)=p-1$ (\Cref{lem:G}(a)), the primitivity of $-r^2$ 
lifts to the nonresidue root $r_-$ itself, via an explicit 
exponent computation in the cyclic group $\F_p^\times$. This 
lift is what drives the entire evaluation of $S(p)$.
...
We show that $\alpha(p)=p-1$ forces $p\equiv 3\pmod 4$ and that 
the nonresidue root $r_-$ of $x^2-x-1$ is a primitive root of 
$\F_p^\times$ (\Cref{lem:G}).
...
We use the standard extension of the quadratic character to 
all of $\F_p$ by setting $\chi_p(0)=0$, and recall that 
...
the residue root by $r_+$, so that $r_-+r_+=1$ and 
$r_-\,r_+=-1$. With this relabelling, the Binet formula may 
...
Throughout this section, assume $p\ge 7$ and $p\equiv\pm 1\pmod 5$.

\begin{lemma}[Rank--order identity]\label{lem:rank-order}
For each root $r$ of $x^2-x-1$ in $\F_p$,
\[
...
\begin{proof}
Since $r_1\,r_2=-1$, we have $r_2=-r_1^{-1}$. From \eqref{eq:binet},
\[
F_m=0\iff r_1^m=r_2^m\iff r_1^{2m}=(-1)^m\iff(-r_1^2)^m=1.
\]
Thus $\alpha(p)=\ord(-r_1^2)$. Since $-r_2^2=(-r_1^2)^{-1}$, 
the two orders agree.
\end{proof}

\begin{lemma}[Primitivity]\label{lem:G}
Assume $\alpha(p)=p-1$. Then:
\begin{enumerate}[label=\textup{(\alph*)}]
\item $p\equiv 3\pmod 4$;
\item exactly one root of $x^2-x-1$ is a quadratic nonresidue, 
and that root is a primitive root of $\F_p^\times$.
\end{enumerate}
\end{lemma}

\begin{proof}
Fix a root $r$ and set $\gamma_0=-r^2$. By \Cref{lem:rank-order}, 
$\ord(\gamma_0)=p-1$, so $\gamma_0^{(p-1)/2}=-1$. Since 
$r\in\F_p^\times$, Fermat's little theorem gives $r^{p-1}=1$, 
whence
\[
\gamma_0^{(p-1)/2}=(-r^2)^{(p-1)/2}
=(-1)^{(p-1)/2}\,r^{p-1}=(-1)^{(p-1)/2}.
\]
Hence $(-1)^{(p-1)/2}=-1$, i.e.\ $p\equiv 3\pmod 4$, proving (a).

For (b), since $p\equiv 3\pmod 4$, $\chi_p(-1)=-1$, so for the 
two roots $r_1,r_2$,
\[
\chi_p(r_1)\,\chi_p(r_2)=\chi_p(r_1 r_2)=\chi_p(-1)=-1.
\]
Hence exactly one root is a residue and one a nonresidue; 
denote them $r_+$ and $r_-$ respectively.

Write $p=4m+3$ and $n=(p-1)/2=2m+1$ (odd). Let 
\[
\gamma:=-r_-^2.
\]
By \Cref{lem:rank-order}, $\gamma$ is primitive. Write 
$r_-=\gamma^k$ for some $k\in\{0,1,\dots,2n-1\}$. Since 
$\gamma$ has order $2n$, we have $\gamma^n=-1$. Thus
\[
r_-^2=-\gamma=\gamma^{n+1}.
\]
Hence
\[
\gamma^{2k}=\gamma^{n+1},
...
\]
Since $n=2m+1$, this congruence is
\[
2k\equiv 2m+2\pmod{4m+2}.
\]
Dividing by $2$ modulo $n$ gives
\[
k\equiv m+1\pmod n.
\]
Therefore the two solutions modulo $2n$ are
\[
k_1=m+1,\qquad k_2=m+1+n=3m+2.
\]
Their sum is $4m+3=p$, which is odd, so exactly one of 
$k_1,k_2$ is odd.

By \Cref{lem:cyclic-character}, applied to the primitive root 
$\gamma$, we have
\[
\chi_p(r_-)=\chi_p(\gamma^k)=(-1)^k.
\]
Since $r_-$ is by definition the nonresidue root, 
$\chi_p(r_-)=-1$, and therefore the admissible exponent $k$ is 
the unique odd one among the two solutions.

We now compute $\gcd(k,2n)$. Since $k$ is odd, $\gcd(k,2)=1$. 
Moreover, using $k\equiv m+1\pmod n$,
\[
\gcd(k,n)=\gcd(m+1,2m+1)
=\gcd(m+1,2m+1-2(m+1))
=\gcd(m+1,-1)=1.
\]
Therefore
\[
\gcd(k,2n)=\gcd(k,2)\gcd(k,n)=1.
\]
Hence
\[
\ord(r_-)=\frac{2n}{\gcd(k,2n)}=2n=p-1,
\]
so $r_-$ is a primitive root of $\F_p^\times$.
\end{proof}

\begin{remark}[The content of \Cref{lem:G}]\label{rem:content-G}
The identity $\alpha(p)=\ord(-r^2)$ from \Cref{lem:rank-order} 
shows tautologically that $-r^2$ is primitive in $\F_p^\times$ 
whenever $\alpha(p)=p-1$. The genuinely non-trivial content of 
\Cref{lem:G}(b) is that the primitivity of $-r^2$ \emph{lifts} 
to the nonresidue root $r_-$ itself, via the explicit exponent 
computation above. This is the structural fact that makes the 
substitution $u=r_-^n$ in Step~II a bijection onto $\F_p^\times$, 
and it is the engine driving the entire evaluation of $S(p)$.
\end{remark}

\begin{corollary}[Order of the residue root]\label{cor:r-plus-order}
Under \Cref{lem:G}, $\ord(r_+)=(p-1)/2$.
\end{corollary}

\begin{proof}
With $n=(p-1)/2=2m+1$, since $r_-$ is primitive we have 
$r_-^n=-1$. From $r_-\,r_+=-1$,
\[
r_+=-r_-^{-1}=r_-^n\,r_-^{-1}=r_-^{n-1}.
\]
Hence $\ord(r_+)=2n/\gcd(2n,n-1)$. Since $n-1=2m$,
\[
...
\begin{remark}
This corollary is not needed for the evaluation of $S(p)$, but 
records the complementary order of the residue root in the 
full-rank regime.
\end{remark}
...
Let $p\ge 7$ with $p\equiv\pm 1\pmod 5$ and $\alpha(p)=p-1$. 
Then
\[
S(p)=\chi_p(r_- - r_+),
\]
...
\tau:=z_0+z_0^{-1}.
\]
By \Cref{lem:phi5-order}, $\Phi_5(z_0)=0$. Dividing by $z_0^2$ gives
\[
...
By \eqref{eq:rank-divides}, $\alpha(p)\mid p-\chi_p(5)$. Since 
$\alpha(p)=p-1$, the case $\chi_p(5)=-1$ would imply 
$p-1\mid p+1$, hence $p-1\mid 2$, impossible for $p\ge 7$. 
Therefore $\chi_p(5)=+1$, so $p\equiv\pm 1\pmod 5$. By 
\Cref{lem:G}, $p\equiv 3\pmod 4$. Combining these congruences gives
\[
...
\Cref{thm:main} was verified for all primes $p<2\times 10^6$ 
...
\item[(i)] the quadratic characters of the two roots of 
$x^2-x-1$ in $\F_p$ were verified against \Cref{lem:G};
\item[(ii)] the multiplicative order of $-r_-^2$ was 
matched against $\alpha(p)$ as required by 
\Cref{lem:rank-order};
...
identity in \Cref{lem:rank-order} identifies $\alpha(p)=p-1$ with 
a primitive-root condition for the element $-r^2\in\F_p^\times$. 
This places the result naturally in the Artin-type family.

\subsection*{A density question}

Among primes $p\le 2\times 10^6$ with $p\equiv\pm 1\pmod 5$ 
(the only class where $\alpha(p)=p-1$ is possible), our data 
show that approximately $35.46\%$ satisfy the full-rank 
condition. \Cref{thm:main} restricts these primes further to 
...
out of the four split classes $\{1,9,11,19\}\pmod{20}$. By 
\Cref{lem:rank-order}, the conjecture is a primitive-root 
statement for the element $-r^2\in\F_p^\times$, restricted 
to the split class $\chi_p(5)=+1$. A Hooley-type analysis in 
this conditional setting, under suitable GRH-type hypotheses 
\cite{Hooley1967}, would be expected to yield a positive 
value of $C_F$ given by an Euler product encoding the 
splitting behaviour of $-r^2$ in the relevant Galois 
extensions. A rigorous treatment lies beyond the scope of 
this paper.
