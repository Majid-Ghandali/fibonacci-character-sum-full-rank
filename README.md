...
| $\chi_p(a)$ | Legendre symbol $\left(\frac{a}{p}\right)$, with the convention $\chi_p(0)=0$ |
| $r_{-}$, $r_{+}$ | Roots of $x^2-x-1$ in $\mathbb{F}_p$, labeled nonresidue / residue |
| Split primes | Primes with $\left(\frac{5}{p}\right)=1$ (equivalently $x^2-x-1$ splits over $\mathbb{F}_p$; $p\equiv\pm1\pmod{5}$) |
...
For every full-rank prime in that range the suite confirms:

- splitting of $x^2-x-1$ over $\mathbb{F}_p$;
- residue / nonresidue labels of the two roots;
- the order identity $\mathrm{ord}(-r_{-}^{2})=p-1$;
- primitivity of $r_{-}$ in $\mathbb{F}_p^{\times}$;
- the structural identity $S(p)=\chi_p(r_{-}-r_{+})$;
- the sign predicted by the fifth-root discriminant criterion.

**Zero exceptions** in that range.
